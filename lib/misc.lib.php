...
function retrieve_xml($url, &$curl_info = NULL) {
  ...
  if ($content === FALSE) {
    trigger_error(check_plain('CURL failed to execute on URL "' . $url . '"'));
    return FALSE;
  }
...
function indentXML($string) {
  ...
  function indentXML_pregCallback($matches) {
    ...
    if (drupal_substr($matches[0], 0, 9) == "<[CDATA[!") {
    ...
    elseif (drupal_substr($matches[0], -2, 1) != '/') {
      $indent++;
...
function beautifulXML($string) {
  ...
  function beautifulXML_pregCallback($matches) {
    ...
    if (drupal_substr($matches[0], 0, 9) == "<![CDATA[") {
      ...
      $content = drupal_substr($matches[0], 9, -3);
      $after   = '</span><span class="delimiter">]]&gt;</span></div>';
    }
    elseif ($matches[0][1] == '?') {
      $before = '<div class="node"><span class="delimiter">&lt;?</span>';
      $content = beautifulXML_tagging(drupal_substr($matches[0], 2, -2));
      $after  = '<span class="delimiter">?&gt;</span></div>';
    }
    elseif ($matches[0][1] == '/') {
      $before = '<span class="delimiter">&lt;/</span>';
      $content = beautifulXML_tagging(drupal_substr($matches[0], 2, -1));
      $after  = '<span class="delimiter">&gt;</span></div>';
    }
    elseif (drupal_substr($matches[0], -2, 1) != '/') {
      $before = '<div class="node"><span class="delimiter">&lt;</span>';
      $content = beautifulXML_tagging(drupal_substr($matches[0], 1, -1));
